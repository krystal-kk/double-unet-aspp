...
from dataset_utils.transforms import RandCrop, RandHorizontalFlip, RandRotate, RandVerticalFlip, ToTensor, Normalize, RandCrop_cell_pred, ToTensor_R
import numpy as np
import random
import pdb
...
class cell_dataset(Dataset):
    ...
    """
    def __init__(self, images_dir, bin_masks_dir, ins_masks_dir, data_list, phase='train', transform = TRANSFORM) -> None:
        super().__init__()
...
class pred_cell_dataset(Dataset):
    def __init__(self, img_list, bin_list, ins_list, transform = TRANSFORM_CELL_PRED) -> None:
        super().__init__()
    ...
    def __getitem__(self, i):
        ...
        sample = self.transform({'img': image, 'bin_mask': bin_mask})
        return sample['img'], sample['bin_mask'], ins_mask


class Cell:
    def __init__(self, imgs_dir, bin_dir, ins_dir, train_list, test_list, target_size=320) -> None:
    ...
    def get_train_dataset(self):
        train_dataset = cell_dataset(self.img_dir, self.bin_dir, self.ins_dir, self.train_list, phase='train')
