...
class UnetASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        e1 = self.Conv1(x)  # size:512  channel:3->64

        e2 = self.Maxpool1(e1) # size:512/2  channel:64
        e2 = self.Conv2(e2)  # size:512/2  channel:64->128

        e3 = self.Maxpool2(e2) # size:512/4  channle:128
        e3 = self.Conv3(e3)  # size:512/4  channel:128->256

        e4 = self.Maxpool3(e3)  # size:512/8  channel:256
        e4 = self.Conv4(e4)  # size: 512/8  channel:256->512

        e5 = self.Maxpool4(e4)  #size:512/16  channel:512
        e5 = self.Conv5(e5)  # size:512/16  channel:512->1024

        aspp_out = self.aspp(e5)  # size:512/16  channel:1024->1024

        d5 = self.Up5(aspp_out)  # size:512/8  channel:1024->512
        d5 = torch.cat((e4, d5), dim=1)  # size:512/8  channel:512->1024

        d5 = self.Up_conv5(d5)  # size:512/8  channel:1024->512

        d4 = self.Up4(d5)  # size:512/4  channel: 512->256
        d4 = torch.cat((e3, d4), dim=1) # size:512/4  channel:256->512
        d4 = self.Up_conv4(d4)  # size:512/4  channel:512->256

        d3 = self.Up3(d4)  # size:512/2  channel: 256->128
        d3 = torch.cat((e2, d3), dim=1)  # size:512/2  channel: 128->256
        d3 = self.Up_conv3(d3)  # size:512/2  channel: 256->128

        d2 = self.Up2(d3)  # size:512  channel: 128->64
        d2 = torch.cat((e1, d2), dim=1)  # size:512  channel: 64->128
        d2 = self.Up_conv2(d2)  # size:512  channel: 128->64

        out = self.Conv(d2)  # size:512  channel: 64->3

        #d1 = self.active(out)
        # print('out shape: ', out.shape)    #out shape:  torch.Size([2, 1, 512, 512])
...
class aspp_conv_block(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, padding=1, stride=1, dilation=1):
        ...
        self.aspp_conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size, stride=stride, padding=padding, bias=False, dilation=dilation),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class ASPP(nn.Module):
    def __init__(self, in_ch, out_ch, dilations=[1, 6, 12, 18]):
        ...
        self.aspp1 = aspp_conv_block(in_ch, out_ch, kernel_size=1, padding=0, dilation=dilations[0])
        self.aspp2 = aspp_conv_block(in_ch, out_ch, kernel_size=3, padding=dilations[1], dilation=dilations[1])
        self.aspp3 = aspp_conv_block(in_ch, out_ch, kernel_size=3, padding=dilations[2], dilation=dilations[2])
        self.aspp4 = aspp_conv_block(in_ch, out_ch, kernel_size=3, padding=dilations[3], dilation=dilations[3])

        self.avg_pool = nn.Sequential(
            nn.AvgPool2d((1, 1)),
            aspp_conv_block(in_ch, out_ch, kernel_size=1, padding=0)
        )
...
class NestedUnetASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        x3_1 = self.conv3_1(torch.cat([x3_0, self.Up(aspp_out)], 1))
...
class AttUnetAspp(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(AttUnetAspp, self).__init__()
...
############### Double Unet + ASPP ###############
class UnetAsppA(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(UnetAsppA, self).__init__()
    ...
    def forward(self, x):
        ...
        encoder_features = [e1, e2, e3, e4]  #? 是从e1开始吗？？？
        ...
        out = self.active(out)
        # print('UnetA out shape', out.shape)
        # print('UnetA output: ', out)

        return x, out, encoder_features


### TODO
class UnetAsppB(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(UnetAsppB, self).__init__()
        ...
        self.Up_conv5 = conv_block(filters[4]+filters[3], filters[3])
        ...
        self.combine_conv = nn.Sequential(
            nn.Conv2d(2, out_ch, kernel_size=1, stride=1, padding=0),
            nn.Sigmoid())
        

    def forward(self, x, output1, encoder_features):
        ...
        d5 = self.Up5(aspp_out)
        d5 = torch.cat((e4, d5, enc4), dim=1)

        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        d4 = torch.cat((e3, d4, enc3), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        d3 = torch.cat((e2, d3, enc2), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        d2 = torch.cat((e1, d2, enc1), dim=1)
        d2 = self.Up_conv2(d2)

        output2 = self.Conv(d2)
        output2 = self.active(output2)

        output = torch.cat([output1, output2], dim=1)
        # print('ouput cat shape: ', output.shape)
        output = self.combine_conv(output)
        # print('final ouput shape: ', output.shape)

        return output



class DoubleUnetAspp(nn.Module):
    def __init__(self):
        super(DoubleUnetAspp, self).__init__()
        self.unetaspp1 = UnetAsppA(in_ch=3, out_ch=1)
        self.unetaspp2 = UnetAsppB(in_ch=3, out_ch=1)

    def forward(self, x):
        x, output1, encoder_features = self.unetaspp1(x)
        output = self.unetaspp2(x, output1, encoder_features)
        return output




############### Double Attention Unet + ASPP ################
class AttentionUnetAsppA(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(AttentionUnetAsppA, self).__init__()
...
class AttentionUnetAsppB(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(AttentionUnetAsppB, self).__init__()
    ...
    def forward(self, x, output1, encoder_features):
        ...
        x4 = self.Att5(g=d5, x=e4)
        # print(x4.shape)
        d5 = torch.cat((x4, d5, enc4), dim=1)
        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        x3 = self.Att4(g=d4, x=e3)
        d4 = torch.cat((x3, d4, enc3), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        x2 = self.Att3(g=d3, x=e2)
        d3 = torch.cat((x2, d3, enc2), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        x1 = self.Att2(g=d2, x=e1)
        d2 = torch.cat((x1, d2, enc1), dim=1)
        d2 = self.Up_conv2(d2)
...
class DoubleAttentionUnetAspp(nn.Module):
    def __init__(self):
        super(DoubleAttentionUnetAspp, self).__init__()
        self.attentionunet_aspp1 = AttentionUnetAsppA(in_ch=3, out_ch=1)
        self.attentionunet_aspp2 = AttentionUnetAsppB(in_ch=3, out_ch=1)

    def forward(self, x):
        x, output1, encoder_features = self.attentionunet_aspp1(x)
        output = self.attentionunet_aspp2(x, output1, encoder_features)
        return output


############### Double NestedUnet + ASPP ################
...
if __name__ == '__main__':
    # net = UnetASPP(in_ch=3, out_ch=1)
    net = DoubleAttentionUnetAspp()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = net.to(device)
    net(torch.randn(1, 3, 512, 512).to(0))
